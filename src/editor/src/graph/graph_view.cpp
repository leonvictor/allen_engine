...
{
// ------ Helpers
Vec2 GetNodeScreenSpaceCenter(const UUID& nodeID)
{
    return (Vec2) ImNodes::GetNodeScreenSpacePos(nodeID) + (Vec2) ImNodes::GetNodeDimensions(nodeID) / 2.0f;
}

Vec2 GetNodeBorderIntersection(const UUID& nodeID, Vec2& start, Vec2& end)
{
...
bool IsConduitCreationAllowed(const EditorGraphNode* pStartNode, const EditorGraphNode* pEndNode)
{
...
void GraphView::SetViewedGraph(EditorGraph* pGraph)
{
    if (pGraph == m_pGraph)
    {
        return;
...
bool GraphView::IsViewingAnimationGraph() const
{
    return m_pGraph != nullptr && dynamic_cast<EditorAnimationGraph*>(m_pGraph) != nullptr;
}

bool GraphView::IsViewingStateMachine() const
{
    return m_pGraph != nullptr && dynamic_cast<EditorAnimationStateMachine*>(m_pGraph) != nullptr;
}
...
// ---- Drawing
void GraphView::DrawNodeContextPopUp()
{
...
void GraphView::DrawCanvasContextPopUp(const TypeRegistryService* pTypeRegistryService, GraphDrawingContext& context)
{
...
void GraphView::Draw(const TypeRegistryService* pTypeRegistryService, GraphDrawingContext& drawingContext)
{
    if (m_pGraph == nullptr)
    {
        return;
...
void GraphView::ClearLinkSelection()
{
...
void GraphView::ClearNodeSelection()
{
...
bool GraphView::IsMouseDragging() const { return m_pImNodesContext->LeftMouseDragging; }

void GraphView::LoadState(const JSON& json, const TypeRegistryService* pTypeRegistryService)
{
}

void GraphView::SaveState(JSON& json) const
{
}
