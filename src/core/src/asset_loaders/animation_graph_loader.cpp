#include "asset_loaders/animation_graph_loader.hpp"
...
namespace aln
{
bool AnimationGraphDefinitionLoader::Load(AssetRequestContext& ctx, AssetRecord* pRecord, BinaryMemoryArchive& archive)
{
...
void AnimationGraphDefinitionLoader::Unload(AssetRecord* pRecord)
{
...
bool AnimationGraphDatasetLoader::Load(AssetRequestContext& ctx, AssetRecord* pRecord, BinaryMemoryArchive& archive)
{
    assert(pRecord->IsUnloaded());
...
void AnimationGraphDatasetLoader::InstallDependencies(AssetRecord* pAssetRecord, const Vector<IAssetHandle>& dependencies)
{
